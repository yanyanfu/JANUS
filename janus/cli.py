...
def get_avg_vision_results(setting_path, out_path):
    ...
    model_name = str(out_path).split('/')[-2]
...
def _output_performance(out_path, results_type):
    ...
    if results_type == 'vision':
        ...
        if str(out_path).split('/')[-1] == 'Lucene_bow':
            ...
            _print_performance(
            f"Textual + Sequential Information, {all_results.iloc[0]['model']}, {technique}", np.mean(text_results['recip_rank'].values),
            np.mean(text_results['avg_precision'].values), np.mean(text_results['first_rank'].values)
            )

    else:
        if results_type == 'comb-bow':
            technique = 'bovw-bow'
            weight = 0.1
        elif results_type == 'comb-bow-seq':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_type', default='text', type=str, choices=['vision', 'text', 'comb-bow', 'comb-bow-seq'],
                        help='The results_type to generate')
    parser.add_argument('--lucene', default=True, type=bool, 
    ...
    if args.results_type == 'vision':
        ...
        get_vis_results (vid_ds, args.arch, args.patch_size, art_path, out_path)
        args.lucene = True if args.results_type == 'comb-bow' else False
        get_txt_results (args.lucene, art_path, out_path)

        vision_model_name = args.arch + str(args.patch_size)
        ir_ranking_path, ir_bow_rankings_path = '', ''
        if args.results_type != 'comb-bow':
        ...
        dl_ranking_path = out_path / 'vision' / vision_model_name / 'avg' / 'avg_rankings.json'
        settings_path = art_path / 'evaluation_settings'
