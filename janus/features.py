...
class DinoExtractor(Extractor):
    ...
    '''Exposed CNNExtractor class used for retrieving features.'''
    def __init__(self, extractor, model_arch):
        super().__init__(extractor)
    ...
    def extract(self, img):
        ...
        if self.model_arch == 'vit_base':
            ...
            intermediate_output = self.extractor.get_intermediate_layers(img.unsqueeze(0), n=4)
            features = torch.cat([x[:, 0] for x in intermediate_output], dim=-1).detach().numpy()
        return features
