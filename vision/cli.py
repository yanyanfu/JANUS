...
def get_vis_results (vid_ds, model_path, out_path):
    ...
    for i in range (args.n_cbs):
        ...
        sample_path = results_path / sample
        ...
        with open(sample_path / f"rankings_{id_name}.pkl", "wb") as f:
            pickle.dump(rankings, f, protocol=pickle.HIGHEST_PROTOCOL)
        with open(sample_path / f"evaluation_metrics_{id_name}.pkl", 'wb') as f:
            pickle.dump(evaluation_metrics, f, protocol=pickle.HIGHEST_PROTOCOL)
        combo.convert_results_format(sample_path, settings_path, out_path, ["Dino"])
